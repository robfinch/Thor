...
void Function::GenerateReturn(Statement* stmt)
{
	...
	if (!IsInline) {
		if (cpu.SupportsLeave) {
			...
			if (currentFn->IsFar)
				GenerateMonadic(op_bra, 0, MakeStringAsNameConst("_FAR_RET", codeseg));
			else
...
void Function::GenerateCoroutineEntry()
{
	...
	cg.GenerateLoadConst(MakeStringAsNameConst("_data_start", dataseg), makereg(regGP));
	ap = GetTempRegister();
...
void Function::GenerateDefaultCatch()
{
	if (!isNocall) {
		...
		GenerateDiadic(op_jsr, 0, makereg(regLR+1), MakeStringAsNameConst("_DEFCAT", codeseg));
