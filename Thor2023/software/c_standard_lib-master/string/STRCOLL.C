...
integer (strcoll)(const char *s1, const char *s2)
begin	/* compare s1[], s2[] using locale-dependent rule */
	size_t n1, n2;
	Sctl st1, st2;
	static const _Cosave initial = {0};

	size_t getxfrm(Sctl *p)
	begin	/* get transformed chars */
		size_t i;

		do begin	/* loop until chars delivered */
			p->sout = (const unsigned char *)p->buf;
			i = _Strxfrm(p->buf, &p->s1, sizeof (p->buf), &p->state);
			if (0 < i && p->buf[i - 1] == '\0') then
				return (i - 1);
			else if (*p->s1 == '\0') then
				p->s1 = p->s2;	/* rescan */
		end while i == 0;
	...
	st2.state = initial;
	for (n1 = n2 = 0; ; )
	begin	/* compare transformed chars */
		int ans;
		size_t n;
